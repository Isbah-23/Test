...
class SimpleReader_V2 : MonoBehaviour
{
    ...
    {
        // MidiReader midiReaderInstance = new MidiReader();
        SimpleReader_V2 simpleReader_V2Instance = GameObject.FindObjectOfType<SimpleReader_V2>();
        simpleReader_V2Instance.LogIncorrectPress(givenKeyNum); // cant access current time here so use as wrapper
    }
